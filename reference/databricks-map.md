...
| **ETL / pipelines** | You performed the three motions — read a file, transform, write a table — twice. Both times on clean data you supplied, running once because you pressed run. Nothing was scheduled, no source system was involved, and you never met the mess that Part 1.6 calls the actual difficulty. Say *"I've done extract-transform-load by hand in a notebook"*, not *"I've built pipelines."* |
